...
from sources.quote_common import KYBER_CHAIN_NAMES, collect_quote_jobs, decimal_price_from_amounts, make_quote, token_decimals, unit_amount
...
class KyberSwapSource(Source):
    ...
    async def _fetch_impl(self) -> list[PriceQuote]:
        chains = [chain for chain in self._chains if chain in KYBER_CHAIN_NAMES]
        return await collect_quote_jobs(self, chains, self._quote)
    ...
    async def _quote(self, chain: str, base: dict[str, str], quote: dict[str, str], amount_raw: str | None = None) -> PriceQuote | None:
        ...
        data = await self._get(
            ...
            params={"tokenIn": base["address"], "tokenOut": quote["address"], "amountIn": amount_raw or unit_amount(base)},
